...
def normalize_name(name):
    ...
    name = re.sub(r'\b(ltd|limited|inc|incorporated|corp|corporation|gmbh|ag|sa|pty|llc|co|auto|trucks|cars|suv|plant|joint venture|export)\b', '', name, flags=re.IGNORECASE)
...
def find_matches(logos, factories):
    ...
    for logo in logos:
        logo_brand_name = logo['brand_name']
        logo_normalized = logo['normalized']

        parentheses_delimiter_pattern = re.compile(
            r'[\(,\/ ]' + re.escape(logo_brand_name) + r'[\),\/ ]', re.IGNORECASE
        )
        strict_word_pattern = re.compile(
            r'\b' + re.escape(logo_normalized) + r'\b'
        )

        for factory in factories:
            factory_matched = False

            if re.search(parentheses_delimiter_pattern, factory['name']):
                factory_matched = True

            if not factory_matched:
                # Check normalized name
                if re.search(strict_word_pattern, factory['normalized']):
                    factory_matched = True
            
            # Additional check: If factory name is contained in logo name (e.g., 'Ford' vs 'Ford Trucks')
            if not factory_matched:
                if logo_normalized and factory['normalized'] and logo_normalized in factory['normalized']:
                    factory_matched = True
...
def import_logos(app):
    """
    Main function to run logo processing and database insertion within the 
